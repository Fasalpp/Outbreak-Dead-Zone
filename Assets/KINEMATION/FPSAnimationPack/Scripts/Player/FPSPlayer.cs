...
namespace KINEMATION.FPSAnimationPack.Scripts.Player
{
    ...
    public class FPSPlayer : MonoBehaviour
    {
        ...
        public List<FPSWeapon> _weapons = new List<FPSWeapon>();
        ...
        private List<int> _weaponsInhandIndex = new List<int>();
        private int _weaponsIndex = 0;
        private bool _hasWeapon = false;
        private int _weaponsInhand = 0;
        ...
        //{
        //    if (_weapons.Count <= 1 || _weaponsInhand <=1) return;
        ...
        {
            if (!_hasWeapon || _weaponsInhand <= 1) return;
            int currentWeapon = 0;
            int preWeapon = currentWeapon;
            float mouseWheelValue = value.Get<float>();
            if (mouseWheelValue == 0f) return;
            //float delay = GetActiveWeapon().OnUnEquipped();
            GetActiveWeapon().gameObject.SetActive(false);
            int currentInHandIndex = _weaponsInhandIndex.IndexOf(_activeWeaponIndex);

            int direction = mouseWheelValue > 0 ? 1 : -1;

            currentInHandIndex += direction;

            if (currentInHandIndex >= _weaponsInhandIndex.Count)
                currentInHandIndex = 0;
            else if (currentInHandIndex < 0)
                currentInHandIndex = _weaponsInhandIndex.Count - 1;

            _activeWeaponIndex = _weaponsInhandIndex[currentInHandIndex];
        ...
        private void Start()
        {
            ...
            hasWeaponSlot = new bool[playerSettings.weaponPrefabs.Count];

            for (int i = 0; i < playerSettings.weaponPrefabs.Count; i++)
            {
                var prefab = playerSettings.weaponPrefabs[i];
        ...
        public void PickupWeapon(int weaponIndex)
        {
            ...
            _weaponsInhand++;
            hasWeaponSlot[weaponIndex] = true;
            _weaponsInhandIndex.Add(weaponIndex);
            _weaponsIndex++;
            if (_hasWeapon)
            {
            ...
            GetActiveWeapon().gameObject.SetActive(true);
            FPSWeapon weapon = GetActiveWeapon().gameObject.GetComponent<FPSWeapon>();
            Debug.Log(weapon);
            if (weapon != null)
            {
                weapon.InitializeAmmo(GetComponentInParent<AmmoInventory>());
            }
            GetActiveWeapon().OnEquipped();
            Invoke(nameof(SetWeaponVisible), 0.05f);
        ...
        private float GetDesiredGait()
        {
            ...
            if (_bSprinting && _moveInput != new Vector2(0.00f,0.00f)) {
                return 2f; }
